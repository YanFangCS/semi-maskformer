...
# semi- or weakly- data don't have ground truth mask annotations, so we can only calculate loss_label for bm 
class HungarianMatcher_Semi(nn.Module):
    ...
    def __init__(self, cost_class: float = 1, cost_mask: float = 1, cost_dice: float = 1, unlabeled_ratio = 0.0):
        """Creates the matcher
    ...
    def memory_efficient_forward(self, outputs, targets):
        ...
        indices = []
        labeled_bs = bs - int(bs * self.unlabeled_ratio)
        ...
        for b in range(bs):
            ...
            # unlabeled data, only calculate labeled_loss for bipart matching
            else:
