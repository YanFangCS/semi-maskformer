...
def _get_ade20k_meta():
    _ids = [k["id"] for k in ADE20K_150_CATEGORIES]
    assert len(_ids) == 150, len(_ids)

    stuff_dataset_id_to_contiguous_id = {k for i,k in enumerate(_ids)}
    stuff_classes = [k["name"] for k in ADE20K_150_CATEGORIES]

    ret = {
        "ade20k_dataset_id_to_contiguous_id": stuff_dataset_id_to_contiguous_id,
        "stuff_classes": stuff_classes,
    }
    return ret


def register_ade20k_2526(root):
    meta = _get_ade20k_meta()
    
    # train
    image_dir = os.path.join(root, "ADE20K_5052", "images")
    gt_dir = os.path.join(root, "ADE20K_5052", "annotations")
    name = "ade20k_5052_sem_seg_train"
    ...
    # validation dataset have been registered in ADE20K-150

def register_ade20k_weakly(root):
    meta = _get_ade20k_meta()

    # train
    image_dir = os.path.join(root, "ADE20K_weakly_5052", "images")
    gt_dir = os.path.join(root, "ADE20K_weakly_5052", "annotations")
    name = "ade20k_5052weakly_sem_seg_train"
...
register_ade20k_2526(_root)
register_ade20k_weakly(_root)