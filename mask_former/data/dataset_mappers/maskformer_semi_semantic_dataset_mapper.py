...
class MaskFormerSemiDatasetMapper:
    ...
    def __init__(
        ...
        *,
        ...
        self.pseudo_labels = np.load("soft_labels.npy", allow_pickle=True).item()
    ...
    def __call__(self, dataset_dict):
        ...
        #     # file_name = file_name.split("/")[-1]
            ...
            file_name = dataset_dict["file_name"]
            instances = Instances(image_size)
            classes = self.pseudo_labels[file_name]["hard_label"]
            classes = np.array(classes)
            # with open("hard_label_info.txt", "a") as f:
            #     print(type(classes), len(classes), classes, file = f)
            classes = classes[classes != self.ignore_label]
            instances.gt_classes = torch.tensor(classes, dtype=torch.int64)
